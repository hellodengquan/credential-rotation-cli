...
from .models import Credential, CredentialStatus, get_type_config


CSV_REQUIRED_FIELDS = ["id", "name", "type", "expires_at"]
CSV_OPTIONAL_FIELDS = [
    "created_at", "rotation_period_days", "warning_days",
    "description", "owner", "service", "secret_value", "tags",
]
    ...
    with open(csv_path, "r", encoding="utf-8") as f:
        ...
        for row_num, row in enumerate(reader, start=2):
            ...
            try:
                ...
                rotation_days = int(row["rotation_period_days"]) if row.get("rotation_period_days") else type_config["rotation_period_days"]
                warning_days = int(row["warning_days"]) if row.get("warning_days") else type_config["warning_days"]
